...
/** @class Main
 * 	Contains entry point and loop functionality of the program
 */
class Main {
	...
	void loop() {
		if (Serial.available() > 0) {					//Wait for serial input
			String serialIn = Serial.readString();		//Catch serial input
			mainController->parseSerial(serialIn);		//Parse serial input
			mainController->printExec();				//Print parsed input
		}
	}
};
